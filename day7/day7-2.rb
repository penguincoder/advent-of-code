...
lnpuarm (910) -> ksbkavj, fmsgmoq, biumgl
bvykqm (98)
xnhlg (76)
mxwdp (81)
jowykql (63)
...
def get_weight(child)
  if @parents[child]
    @weights[child] + @parents[child].inject(0) { |sum, x| sum + get_weight(x) }
  else
    @weights[child]
  end
end

@parents.each do |p, childs|
  @balance[p] = childs.map { |x| get_weight(x) }
end

puts "@parents #{@parents}"
puts "@weights #{@weights}"
puts "@balance #{@balance}"
puts "root is #{@parents.keys.detect { |x| !@parents.values.flatten.include?(x) }}"
puts "imbalance #{@balance.select { |key, vals| vals.uniq.size > 1 }}"
