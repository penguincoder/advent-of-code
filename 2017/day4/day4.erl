...
split_string_into_words(Input)
  -> re:split(Input, " ").

list_has_duplicates(Words)
  -> WordsList = split_string_into_words(Words),
     WordsLength = length(WordsList),
     RemoveWordFromList = fun(X) -> lists:filter(fun(Word) -> not string:equal(Word, X) end, WordsList) end,
     lists:any(fun(Word) -> length(RemoveWordFromList(Word)) < WordsLength - 1 end, WordsList).

valid_phrase_count(Lines)
  -> lists:foldl(fun(Line, Sum) -> HasDuplicates = list_has_duplicates(Line), if HasDuplicates -> Sum; true -> Sum + 1 end end, 0, Lines).

split_string_by_lines(Input)
  -> re:split(Input, "\n").

list_has_anagrams(Words)
  -> WordsList = split_string_into_words(Words),
     lists:any(fun(Word) ->
                   FilteredList = lists:filter(fun(X) -> not string:equal(Word, X) end, WordsList),
                   if
                     length(FilteredList) + 1 < length(WordsList) ->
                       true;
                     true ->
                       WordSize = string:length(Word),
                       SameSizeList = lists:filter(fun(X) -> string:length(X) == WordSize end, FilteredList),
                       WordChars = re:split(Word, ""),
                       WordCountingFun = fun(X, Sum) -> maps:put(X, maps:get(X, Sum, 0)+1, Sum) end,
                       CharCounts = lists:foldl(WordCountingFun, maps:new(), WordChars),
                       lists:any(fun(CheckWord) ->
                                     CheckWordCharCounts = lists:foldl(WordCountingFun, maps:new(), re:split(CheckWord, "")),
                                     not lists:any(fun(X) -> maps:get(X, CharCounts) /= maps:get(X, CheckWordCharCounts, -1) end, WordChars)
                                 end, SameSizeList)
                  end
               end, WordsList).

valid_phrases_without_anagrams(Lines)
  -> lists:foldl(fun(Line, Sum) -> HasAnagrams = list_has_anagrams(Line), if HasAnagrams -> Sum; true -> Sum + 1 end end, 0, Lines).

start()
  -> io:fwrite("~B~n", [valid_phrase_count(["aa bb cc dd ee","aa bb cc dd aa","aa bb cc dd aaa"])]),
     io:fwrite("~B~n", [valid_phrase_count(split_string_by_lines("oaoe rxeq vssdqtu xrk cjv yaoqp loo
...
qrwu mgnw hvflf ytspp mco ikvbqg fflvh wts cbbf"))]),
     io:fwrite("~w~n", [valid_phrases_without_anagrams(["abcde fghij","abcde xyz ecdab","a ab abc abd abf abj","iiii oiii ooii oooi oooo","oiii ioii iioi iiio"])]),
     io:fwrite("~w~n", [valid_phrases_without_anagrams(split_string_by_lines("oaoe rxeq vssdqtu xrk cjv yaoqp loo
mveua dogbam szydvri hyzk lbega abzqw xwjn wniug kwbre
npaoy uivpxwd oynpa rcdk uixpvdw
yserir iikzcm ieuroca iuwcfov rvb giti crdpdcv mxpps
spyuhgo lucasl ucllsa bymnjig yflbv nxitmlf
